      ...
      <dx-item>
        <template #default>
          <div class="login-link">
            <router-link to="/login">{{ $t('app.login.return-sign-in') }}</router-link>
          </div>
        </template>
      </dx-item>
      <template #resetTemplate>
        <div>
          <span class="dx-button-text">
              ...
              <span v-if="!loading">{{ $t("app.login.reset-password") }}</span>
          </span>
        </div>
      </template>
    </dx-form>
...
const notificationText = i18n.t('app.login.reset-password-notification');
